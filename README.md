...
* Ritualism: This requires a sufficient Magic skill. It works by having components to create magical effects.<ul>
  <li>Centerblock: The centerblock is activated to start the ritual by redstone or barehandedly. The centerblock will require a certain number of lapis lazuli to start the ritual, which can be fed into it using a hopper. 
  <li>Zone: This is a ring of salt around the ritual; salt can only be placed by people with the magic skill. This is where the ritual will apply its effects, within this zone. The centerblock can be anywhere within this zone.
  <li>Obelisk: these are four pillars of granite at the corners of the centerblock that determine how high the ritual affects; the ritual will naturally affect two blocks high (the two lowest blocks do not need to be granite) and as far up as all four pillars go; differing heights prevents the ritual from working.
    <li>Temporal Pillars: This is a pillar of blocks on top of the centerblock; it does not need to touch the centerblock but must occupy the same column, be unbroken, and within the zone. Each coal block increases the ritual duration by four seconds; each iron block by twenty seconds; each redstone block by a minute and a half; each emerald block by two minutes; gold by four minutes; diamond by eight minutes. Blocks must be placed with the lowest time blocks on bottom and others above, and each temporal pillar must have the same configuration.
    <li>Sigils: Sigils are made within the Zone of the ritual to cause its effects. Sigils can be placed by a person with magic skill by right clicking the tops of blocks inside a zone with certain items. 
      <li>Target Sigil: A target sigil is a sigil that determines where the ritual's effects target within its zone. It must be placed adjacent to the column of the world the centerblock is in (it can be at any y value within the zone but must be adjacent to the centerblock) <ul>
        <li>All sigil: Selects all living entities within the zone, made using a bone
        <li>Name Sigil: A name sigil, made by using a nametag, will scan for all entities in the zone which are of the given name and apply its effects to them. This includes players and named mobs.
          <li>Type Sigil: A type sigil, made using a scraper, will scan for all entities of the specified type
            <li>Block-type Sigil: A block-type sigil, made by right-clicking using a block-bag, will scan for all blocks of the state stored in it (it will ignore the tile entity state, however).
            <li>Identity Sigil: An identity sigil, made using lycopodium, scans for all entities wearing the form of the entity stored in it (usually this is one entity, but it could potentially be more)
            <li>Trait Sigil: A trait sigil scans for all entities with the data tags and entity selector(either can be left blank) that are inputted into its GUI. It will display the datatags of the opener, or, if the opener used a lycopodium powder on it, the mob supplied to it using the powder, as reference, so the player will find it easier to write the right tags (the selector, however, will have no references. It also will be unable to select mobs outside the zone regardless of parameters).
        </ul>
        <li>Effect Sigil: 
  </ul>
* Alchemy: This requires a sufficient alchemy skill<ul>
  <li>
  </ul>
* 
