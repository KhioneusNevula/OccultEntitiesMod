...
public class IdeaMemory<E extends LivingEntity> extends Memory<E> {
	...
	public void initialize() {
		Set<SapientTask<? super E>> tasques = Sets.newHashSet(TaskType.getValues()).stream()
				...
				.filter((m) -> m.getDoerClass().isAssignableFrom(this.getOwner().getClass()))
	...
	public IdeaMemory(E owner, Dynamic<?> dyn) {
		...
		if (dyn.get("task").get().isPresent()) {
			this.doTask = TaskType.deserialize(SapientInfo.get(owner).getAutonomy(), dyn.get("task").get().get());
		}
	}
	...
	public void affectCitizen(E en) {
		if (doTask != null) {
			SapientInfo.get(en).getAutonomy().considerTask(2, doTask, Necessity.UNNECESSARY);
		}
	}
