...
public class Emotions extends InformationHolder {
	...
	public <T extends LivingEntity> void addMood(Mood<?> mood, int time, T entity) {
		if (mood.isForClass(entity.getClass())) {
			...
			DoubleSupplier sup = () -> entity.getRNG().nextDouble() * 2 - 1;
			Networking.sendToTracking(new TaskParticles(ParticleTypes.CRIT, entity.getPosX() + sup.getAsDouble(),
					entity.getPosYEye() + sup.getAsDouble(), entity.getPosZ() + sup.getAsDouble(), entity.dimension,
					sup.getAsDouble(), sup.getAsDouble(), sup.getAsDouble(), true, false, false), entity);
		}
	}
