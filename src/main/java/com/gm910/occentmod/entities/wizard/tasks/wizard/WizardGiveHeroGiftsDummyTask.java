...
public class WizardGiveHeroGiftsDummyTask extends Task<WizardEntity> {
	private static final Map<VillagerProfession, ResourceLocation> GIFTS = Util.make(Maps.newHashMap(),
			(p_220395_0_) -> {
				...
				p_220395_0_.put(VillagerProfession.CARTOGRAPHER,
						LootTables.GAMEPLAY_HERO_OF_THE_VILLAGE_CARTOGRAPHER_GIFT);
				...
				p_220395_0_.put(VillagerProfession.LEATHERWORKER,
						LootTables.GAMEPLAY_HERO_OF_THE_VILLAGE_LEATHERWORKER_GIFT);
				...
				p_220395_0_.put(VillagerProfession.WEAPONSMITH,
						LootTables.GAMEPLAY_HERO_OF_THE_VILLAGE_WEAPONSMITH_GIFT);
			});
	private int cooldown = 600;
	private boolean done;
	private long startTime;

	public WizardGiveHeroGiftsDummyTask(int p_i50366_1_) {
		super(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleStatus.REGISTERED, MemoryModuleType.LOOK_TARGET,
				MemoryModuleStatus.REGISTERED, MemoryModuleType.INTERACTION_TARGET, MemoryModuleStatus.REGISTERED,
				MemoryModuleType.NEAREST_VISIBLE_PLAYER, MemoryModuleStatus.VALUE_PRESENT), p_i50366_1_);
	}
	...
	protected boolean shouldContinueExecuting(ServerWorld worldIn, WizardEntity entityIn, long gameTimeIn) {
		return false;// this.hasNearestPlayer(entityIn) && !this.done;
	}
	...
	private void giveGifts(WizardEntity p_220398_1_, LivingEntity p_220398_2_) {
	...
	private List<ItemStack> getGifts(WizardEntity p_220399_1_) {
		if (p_220399_1_.isChild()) {
			return ImmutableList.of(new ItemStack(Items.POPPY));
		} else {
			VillagerProfession villagerprofession = null;// p_220399_1_.getVillagerData().getProfession();
			if (GIFTS.containsKey(villagerprofession)) {
				LootTable loottable = p_220399_1_.world.getServer().getLootTableManager()
						.getLootTableFromLocation(GIFTS.get(villagerprofession));
				LootContext.Builder lootcontext$builder = (new LootContext.Builder((ServerWorld) p_220399_1_.world))
						.withParameter(LootParameters.POSITION, new BlockPos(p_220399_1_))
						.withParameter(LootParameters.THIS_ENTITY, p_220399_1_).withRandom(p_220399_1_.getRNG());
	...
	private boolean hasNearestPlayer(WizardEntity p_220396_1_) {
		return this.getNearestPlayer(p_220396_1_).isPresent();
	}

	private Optional<PlayerEntity> getNearestPlayer(WizardEntity p_220400_1_) {
	...
	private boolean isCloseEnough(WizardEntity p_220401_1_, PlayerEntity p_220401_2_) {
