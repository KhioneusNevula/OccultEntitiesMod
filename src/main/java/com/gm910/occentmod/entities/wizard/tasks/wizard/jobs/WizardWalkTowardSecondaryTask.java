...
public class WizardWalkTowardSecondaryTask extends Task<WizardEntity> {
	...
	public WizardWalkTowardSecondaryTask(MemoryModuleType<List<GlobalPos>> p_i50340_1_, float p_i50340_2_,
			int p_i50340_3_, int p_i50340_4_, MemoryModuleType<GlobalPos> p_i50340_5_) {
		super(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleStatus.REGISTERED, p_i50340_1_,
				MemoryModuleStatus.VALUE_PRESENT, p_i50340_5_, MemoryModuleStatus.VALUE_PRESENT));
	...
	protected boolean shouldExecute(ServerWorld worldIn, WizardEntity owner) {
		...
		if (optional.isPresent() && optional1.isPresent()) {
			...
			if (!list.isEmpty()) {
				...
				return this.field_220579_g != null
						&& Objects.equals(worldIn.getDimension().getType(), this.field_220579_g.getDimension())
						&& optional1.get().getPos().withinDistance(owner.getPositionVec(),
								(double) this.field_220577_e);
			}
		}
	...
	protected void startExecuting(ServerWorld worldIn, WizardEntity entityIn, long gameTimeIn) {
		if (gameTimeIn > this.field_220578_f && this.field_220579_g != null) {
			entityIn.getBrain().setMemory(MemoryModuleType.WALK_TARGET,
					new WalkTarget(this.field_220579_g.getPos(), this.field_220575_c, this.field_220576_d));
