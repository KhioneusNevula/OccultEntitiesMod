...
public class WizardGatherPOITask extends Task<CreatureEntity> {
	...
	public WizardGatherPOITask(PointOfInterestType p_i50374_1_, MemoryModuleType<GlobalPos> p_i50374_2_,
			boolean p_i50374_3_) {
	...
	protected void startExecuting(ServerWorld worldIn, CreatureEntity entityIn, long gameTimeIn) {
		...
		Stream<BlockPos> stream = pointofinterestmanager.findAll(this.field_220604_a.getPredicate(), predicate,
				new BlockPos(entityIn), 48, PointOfInterestManager.Status.HAS_SPACE);
		...
		if (path != null && path.reachesTarget()) {
			...
			pointofinterestmanager.getType(blockpos).ifPresent((p_225441_5_) -> {
				...
				entityIn.getBrain().setMemory(this.field_220605_b,
						GlobalPos.of(worldIn.getDimension().getType(), blockpos));
