...
public class Networking {
	 ...
	 * @param excluded
	 */
	public static void sendNear(TaskMessage msg, double x, double y, double z, DimensionType dim, int radius, @Nullable ServerPlayerEntity excluded) {
		ModChannels.INSTANCE.send(PacketDistributor.NEAR.with(()->new PacketDistributor.TargetPoint(excluded, x, y, z, radius, dim)), msg);
	}
	 ...
	 * @param excluded
	 */
	public static void sendNear(ModTask msg, double x, double y, double z, DimensionType dim, int radius, @Nullable ServerPlayerEntity excluded) {
		ModChannels.INSTANCE.send(PacketDistributor.NEAR.with(()->new PacketDistributor.TargetPoint(excluded, x, y, z, radius, dim)), new TaskMessage(msg));
	}
	...
	public static class TaskMessage {
		...
		public void encode(PacketBuffer buf) {
			...
			if (nbt != null && !nbt.isEmpty()) buf.writeCompoundTag(nbt);
		}
		...
		public void handle(Supplier<NetworkEvent.Context> context) {
			...
			if (tasque == null) {
				...
				if (tasque.isLClient() && to.isClient() || tasque.isLServer() && to.isServer()) {
					System.out.println("Handling task sent from " + from + " on " + to + ": " + tasque);
